...
export function StadiumHero() {
  ...
  const moodData = {
    ...
    excited: { label: "Energetic", color: "text-electric-cyan bg-electric-cyan/10 border-electric-cyan/20 animate-pulse" },
    tense: { label: "Tense Queue", color: "text-crowd-orange bg-crowd-orange/10 border-crowd-orange/20" },
    celebrating: { label: "Celebrating", color: "text-victory-gold bg-victory-gold/10 border-victory-gold/20 animate-bounce" },
  };

  return (
    <Card variant="glass" className="w-full text-left relative overflow-hidden p-6 border-neutral-800 bg-neutral-950/60 backdrop-blur-xl">
      ...
      <div className={`absolute top-0 right-0 w-80 h-80 rounded-full blur-[100px] pointer-events-none -z-10 transition-colors duration-500 ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
        ...
        <div className="md:col-span-7 grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div className="p-3 rounded-xl border border-neutral-900 bg-neutral-950/40 text-xs">
            ...
            <div className={`inline-flex px-2 py-0.5 rounded-md text-xs font-bold capitalize border ${moodData[crowdMood].color}`}>
      ...
      <div className="mt-6 pt-4 border-t border-neutral-900">
        <div className="relative flex items-center justify-between w-full">
          ...
          {milestones.map((m: { label: string; phases: MatchPhase[] }, idx: number) => {
            const isCompleted = milestones.slice(0, idx).some((prev: { label: string; phases: MatchPhase[] }) =>
