    ...
    <div className="fixed bottom-6 right-6 z-40 max-w-sm w-[320px] hidden md:block rounded-2xl border border-neutral-900 bg-neutral-950/80 backdrop-blur-md p-4 text-left shadow-2xl font-mono text-[9px] space-y-3">
      ...
      <div className="flex items-center justify-between border-b border-neutral-900 pb-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-cyber-green font-bold text-[8px] tracking-widest uppercase">Live Link</span>
        </div>
      </div>
      ...
      <div className="space-y-2.5 h-[110px] overflow-hidden flex flex-col justify-end">
        {logs.map((log) => {
          ...
          if (log.type === "AI PREDICT") badgeColor = "text-victory-gold border-victory-gold/20 bg-victory-gold/5";
          if (log.type === "DISPATCH") badgeColor = "text-rose-500 border-rose-500/20 bg-rose-500/5";
          if (log.type === "SRE HEALTH") badgeColor = "text-cyber-green border-cyber-green/20 bg-cyber-green/5";

          return (
            <div key={log.id} className="space-y-1 animate-slide-up border-l border-neutral-900 pl-2">
              <div className="flex items-center gap-1.5 text-neutral-500 font-bold">
                <span>{log.time}</span>
                <span className={`px-1.5 py-0.5 rounded border text-[7px] tracking-wider uppercase font-semibold ${badgeColor}`}>
                  {log.type}
                </span>
              </div>
