...
export function MatchMemory() {
  ...
  return (
    <Card variant="glass" className="border-neutral-850 bg-neutral-950/60 backdrop-blur-xl text-left font-sans">
      <CardHeader className="pb-3 border-b border-neutral-900 flex flex-row items-center justify-between">
        ...
        <div className="h-10 w-10 rounded-full overflow-hidden border border-victory-gold/30 flex-shrink-0">
          <Image src="/images/family_avatar.jpg" alt="Family Avatar" width={40} height={40} className="h-full w-full object-cover" />
        </div>
      </CardHeader>
            ...
            {timeline.map((t, idx) => {
              ...
              return (
                <div key={idx} className="relative">
                  ...
                  <span className={`absolute -left-7 top-0.5 flex h-4 w-4 rounded-full border bg-neutral-950 items-center justify-center ${t.color}`}>
        ...
        <div className="w-full sm:w-[220px] flex-shrink-0 space-y-3">
          ...
          <div className="flex flex-col gap-2">
            {achievements.map((a, idx) => {
              const Icon = a.icon;
              return (
                <div key={idx} className={`p-2.5 rounded-xl border flex items-start gap-2.5 text-[10px] leading-relaxed ${a.color}`}>
